...
for week in data['weeks']:
    ...
    week_with_leading_zeroes = utils.int_with_leading_zeroes( week, len(str(max_weeks)) )
    ...
    for i in range(len(data['weeks'][week])):
        ...
        number_with_leading_zeroes = utils.int_with_leading_zeroes( i, len(str(max_problems)) )
        dirname = '_'.join([ number_with_leading_zeroes, utils.prep_filename( problem ) ])

        # Add a bullet point of: * [Link Preview](Link Body)
        week_readme_contents += '* {}\n'.format( utils.make_md_link( dirname, problem ) )
        ...
        if not readme_Path.exists():
            ...
            readme_Path.write_text( '# {}\n\n{}\n'.format( data['weeks'][week][i], solutions_line ) )
        ...
        for i in range(len(content)):
            ...
            # find all the solutions below it
